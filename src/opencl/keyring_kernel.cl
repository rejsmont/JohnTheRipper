...
inline void symkey_generate_simple(__global const unsigned char *password, int n_password, __global const unsigned char *salt, int n_salt, int iterations, __global unsigned char *key, __global unsigned char *iv)
{
	...
	__global unsigned char *at_iv, *at_key;


	_memcpy(lpassword, password, n_password);
	_memcpy(lsalt, salt, n_salt);
	...
	for (pass = 0;; ++pass) {
		...
		if (lpassword) {
			sha256_update(&ctx, lpassword, n_password);

		}
		if (salt && n_salt) {
			sha256_update(&ctx, lsalt, n_salt);
		}
...
__kernel void keyring(__global const keyring_password * inbuffer,
                  __global keyring_hash * outbuffer, __global const keyring_salt * salt)
{
	uint32_t idx = get_global_id(0);

	symkey_generate_simple(inbuffer[idx].v, inbuffer[idx].length, salt->salt, salt->length, salt->iterations, outbuffer[idx].key, outbuffer[idx].iv);
}
